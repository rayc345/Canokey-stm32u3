...
static void _prep_out_transaction(ccidd_interface_t *p_itf) {
  ...
  if (usbd_edpt_busy(BOARD_TUD_RHPORT, p_itf->ep_out)) return;

  // Prepare for incoming data but only allow what we can store in the ring buffer.
  uint16_t max_read = tu_fifo_remaining(&p_itf->rx_ff);
  if (max_read >= CFG_TUD_CCID_EPSIZE) {
    usbd_edpt_xfer(BOARD_TUD_RHPORT, p_itf->ep_out, p_itf->epout_buf, CFG_TUD_CCID_EPSIZE);
  }
}
...
uint32_t tud_ccid_n_write(uint8_t itf, const void *buffer, uint32_t bufsize) {
  ...
  TU_ASSERT(usbd_edpt_xfer(BOARD_TUD_RHPORT, p_itf->ep_in, (uint8_t *)buffer, bufsize), 0);
